...
上記 Goals の達成はベンチマーク条件（7.2 節）に基づく NPS（Nodes Per Second）の定量比較により確認する。具体的な確認基準は以下の通りである:

| 確認項目 | 基準 |
|---------|------|
| 計測条件 | 7.2 節のベンチマーク条件（3局面 × 2深さ × 5回計測、中央値採用） |
| 比較基準 | Phase 2 完了時点（PR #59 マージ直後）のコードベースで計測した NPS |
| 達成条件 | 全計測条件において Phase 2 ベースラインに対して NPS が改善していること |
| 目標値 | 同一局面・同一深さで NPS 2〜5 倍の改善（7.2 節の総合効果目標） |

**実装 PR へのエビデンス記載要件**: 実装 PR には以下のエビデンスを必ず記載すること。エビデンスの記載がない PR はマージしない。

- 各実装ステップ（Step 1〜3）完了時点のベンチマーク結果（NPS 値）
- Phase 2 ベースラインとの NPS 比較表
- 全計測条件（3局面 × 2深さ）の個別結果と、Phase 2 ベースラインに対する改善率
...
- **実装 PR へのベンチマークエビデンス記載**: 各ステップの実装 PR には、ベンチマーク条件（7.2 節）に基づく NPS 比較結果を必ず記載すること（Goals 達成の確認方法を参照）。エビデンスのない PR はマージ不可とする。
